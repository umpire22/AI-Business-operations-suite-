...
# ---------- Branding with Logo ----------
try:
    logo = Image.open("logo.png")  # place your generated logo.png in the project root
    st.image(logo, width=120)
except:
    st.markdown("🤖")

st.markdown('<p class="main-title">AI Business Operations Suite</p>', unsafe_allow_html=True)
st.markdown('<p class="sub-title">Your 5-in-1 AI Assistant for Smarter Workflows</p>', unsafe_allow_html=True)
st.markdown("---")
...
with st.sidebar:
    try:
        ...
        st.markdown("🤖")
    st.title("📌 Navigation")
    st.markdown("Select an AI Agent below to get started:")
...
# ---------- Agent 3: Medical Record Management ----------
    ...
    uploaded_file = st.file_uploader("Upload Medical Records (CSV/XLSX with Patient_ID column)", type=["csv", "xlsx"])
    pasted_csv = st.text_area("Paste CSV content (optional)", height=150)
        ...
        if "Patient_ID" in df.columns:
            ...
            if st.button("Search Patient"):
                ...
                if results.empty:
                    ...
                    st.download_button("Download Patient Record", data=df_to_csv_bytes(results), file_name=f"{patient_id}_record.csv", mime="text/csv")
        else:
            st.warning("Dataset must include a 'Patient_ID' column.")
    else:
        st.info("Upload medical records CSV/XLSX with a Patient_ID column.")
...
# ---------- Agent 4: Customer Feedback Analysis ----------
    ...
    st.markdown('<div class="agent-sub">Sentiment analysis for individual or bulk feedback with charts.</div>', unsafe_allow_html=True)
    st.markdown("<div class='card'>", unsafe_allow_html=True)
...
# ---------- Agent 5: Order Status Tracking ----------
    ...
    st.markdown('<div class="agent-sub">Enter Order IDs and get simulated updates. Batch CSV supported.</div>', unsafe_allow_html=True)
    st.markdown("<div class='card'>", unsafe_allow_html=True)
